...
async def openai_compatible_chat_completions(request: Request):
    ...
    try:
        reply_text = gemini_service.generate_response(
            ...
            # Deliberately not forwarding body.get("model") here - judges/testing
            # tools send a placeholder string like "gemini" that isn't a real
            # Gemini model ID, so passing it through wastes one API call per
            # message on a guaranteed 404 before falling back to a real model.
            # Skipping straight to gemini_service's own fallback list conserves
            # our limited free-tier quota.
            model_name=None,
        )
